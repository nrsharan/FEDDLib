...
int main(int argc, char *argv[])
{
    ...
    Teuchos::TimeMonitor::setStackedTimer(stackedTimer);
    {
        ...
        p1Partitioner.readAndPartition(15,"dm",true); // Needs to be transformed from cm to mm
        // since the target unit is cm, we cheat a little, because we want it to be in mm finally. We say it is dm to get it scaled by 10
        ...
        bcFactoryStructure->addBC(zeroDirichlet3D, 7, 0, domainStructure, "Dirichlet_Z", dimension);    // z=0, includes outer ring
        bcFactoryStructure->addBC(zeroDirichlet3D, 8, 0, domainStructure, "Dirichlet_Z", dimension);    // z=0.5
        bcFactoryStructure->addBC(zeroDirichlet3D, 9, 0, domainStructure, "Dirichlet_Z", dimension);    // z=0, inner ring
        bcFactoryStructure->addBC(zeroDirichlet3D, 10, 0, domainStructure, "Dirichlet_Z", dimension);    // z=0.5 inner ring
        ...
        bcFactory->addBC(zeroDirichlet3D, 10, 0, domainStructure, "Dirichlet_Z", dimension);
        ...
        bcFactoryDiffusion->addBC(inflowChem, 6, 0, domainDiffusion, "Dirichlet", 1, parameter_vec); // Inflow through inner wall
        bcFactoryDiffusion->addBC(inflowChem, 9, 0, domainDiffusion, "Dirichlet", 1, parameter_vec); // z=0, inner ring on innter wall
        bcFactoryDiffusion->addBC(inflowChem, 10, 0, domainDiffusion, "Dirichlet", 1, parameter_vec); // z=0.5 inner ring
        ...
        bcFactoryDiffusion->addBC(inflowChem,11, 0, domainDiffusion, "Dirichlet", 1, parameter_vec); // Inflow through outer wall
        bcFactoryDiffusion->addBC(inflowChem, 13, 0, domainDiffusion, "Dirichlet", 1, parameter_vec); // x-z points
        bcFactoryDiffusion->addBC(inflowChem, 14, 0, domainDiffusion, "Dirichlet", 1, parameter_vec); // y-z points

        bcFactory->addBC(inflowChem, 11, 1, domainDiffusion, "Dirichlet", 1, parameter_vec);
...
void loadFunction(double *x, double *res, double *parameters)
{
    ...
    if (surfaceFlag == 6) // If the surface flag is 5
