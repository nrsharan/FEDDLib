...
void rhsImp(double* x, double* res, double* parameters){
	...
	if(parameters[0] <= 2.0){
   		 res[0] = (x[0]/r)*parameters[1]*sin(M_PI *1./2*(parameters[0]));
  	 	 res[1] = (x[1]/r)*parameters[1]*sin(M_PI *1./2*(parameters[0]));
	}
	else{ 
...
//                domainP2->buildP2ofP1Domain(domainP1);
            ...
            double finalTimeRamp = parameterListAll->sublist("Timestepping Parameter").get("Final time force",1.0);
            double degree = 0;
