...
/*!
 main of unsteadynoninearDiffusionReaction problem
...
int main(int argc, char *argv[]) {
    ...
    
    {
        ...
        exPara->setup("Flags", domain->getMesh(), FEType);
        
        exPara->addVariable(exportSolutionConst, "Flags", "Scalar", 1, domain->getMapUnique(), domain->getMapUniqueP2());
		...
		for(int i=0; i<dim; i++){
			diffusionTensor[0][0] =1./150;
		    diffusionTensor[1][1] =1./300;
		    diffusionTensor[2][2] =1./150;
			
			/*if(i>0){
				diffusionTensor[i][i-1] = -1./30;
				diffusionTensor[i-1][i] = -1./30;
			}
			else
				diffusionTensor[i][i+1] = -1./30;*/			
		}
