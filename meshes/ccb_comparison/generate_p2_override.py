...
FEDDLib only reads P1 medit meshes (hollow_cylinder_p1.mesh) and builds P2
with straight edges, while this test's
...
FLAG lines give every node of svMultiPhysics's inner (5) and outer (4) walls,
bottom/top faces (2/3 = Dirichlet_Z), 6-node pin faces (13/16 = Dirichlet_Y,
14 = Dirichlet_X) and the wall end rings (inner 7/8, outer 6/9) the flag the
ccb_hollow_cylinder_cmm driver uses for its structure and concentration BCs.
The driver resets these flags on all other nodes, so the node sets are identical
to svMultiPhysics's faces.

Requires: the `meshio` Python package.
Usage: python3 generate_p2_override.py <svMultiPhysics tests/cases/def_diffu/hollow_cylinder>
"""
...
if len(sys.argv) != 2:
    sys.exit("usage: generate_p2_override.py <svMultiPhysics tests/cases/def_diffu/hollow_cylinder>")
HERE = sys.argv[1]
...
P1_MESH = os.path.join(THIS_DIR, "hollow_cylinder_p1.mesh")
OUT = os.path.join(THIS_DIR, "hollow_cylinder_p2_override.txt")
...
def check_effective_directions():
    ...
    for name, direction in re.findall(
        ...
        if len(comps) == 1 and comps[0] == 3:
            continue  # concentration BC
...
def main():
    ...
    # End rings of the walls (on the bottom/top faces too): inner 7 (z=0) / 8 (z=top),
    # outer 6 (z=0) / 9 (z=top), written last so they take precedence.
    r = np.hypot(X[:, 0], X[:, 1])
    zmin, zmax = X[:, 2].min(), X[:, 2].max()
    rings = {7: (r.min(), zmin), 8: (r.min(), zmax), 6: (r.max(), zmin), 9: (r.max(), zmax)}
    for flag, (radius, z) in rings.items():
        nodes = [p for p in range(len(X)) if abs(r[p] - radius) < 1e-9 and abs(X[p, 2] - z) < 1e-9]
        counts[f"ring{flag}"] = len(nodes)
        flag_lines += [(p, flag) for p in nodes]
